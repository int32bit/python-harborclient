...
def discover_version(client, requested_version):
    ...
    if requested_version == APIVersion('2.0'):
        if (server_start_version == APIVersion('2.1') or
            (server_start_version.is_null() and server_end_version.is_null())):
            return APIVersion('2.0')
        else:
            raise exceptions.UnsupportedVersion(
                _("The server isn't backward compatible with Harbor V2 REST "
                  "API"))
...
def _warn_missing_microversion_header(header_name):
    ...
    LOG.warning(
        _LW("Your request was processed by a Harbor API which does not support "
